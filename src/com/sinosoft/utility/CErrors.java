...
public class CErrors implements Cloneable
{
	...
	public String getErrContent()
	{
		...
		for (int i = 0; i < vErrors.size(); i++)
		{
			...
			if (tError.errorType.equals(CError.TYPE_FORBID))
			{
				forbitContent = forbitContent + "  " + "（错误编码：" + tError.errorNo + "）" + tError.errorMessage + "\n";
			}
			else if (tError.errorType.equals(CError.TYPE_NEEDSELECT))
			{
				needSelectContent = needSelectContent + "  " + "（错误编码：" + tError.errorNo + "）" + tError.errorMessage
						+ "\n";
			}
			else if (tError.errorType.equals(CError.TYPE_ALLOW))
			{
				allowContent = allowContent + "  " + "（错误编码：" + tError.errorNo + "）" + tError.errorMessage + "\n";
			}
			else
			{
				unknowContent = unknowContent + "  " + "（错误编码：意外错误）" + tError.errorMessage + "\n";
			}
		}
